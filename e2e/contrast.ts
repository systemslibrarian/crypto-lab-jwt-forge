 ...
 *    files the node under `incomplete`. `.cl-hero-why` is a near-transparent
 *    `color-mix(in oklab, ...)` wash, and the diff rows are translucent rgba
 *    tints, so their text composites onto whatever is behind rather than onto a
 ...
 *    `.raw-token` is `white-space: pre` with `overflow-x: auto` around a JWT that
 *    never wraps, so at 380px most of the token is clipped away. Content scrolled past the client box is not dimmed or partly
